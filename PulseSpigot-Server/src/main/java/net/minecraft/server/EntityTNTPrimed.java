...
public class EntityTNTPrimed extends Entity {
    ...
    // PaperSpigot end
        ...
        this.loadChunks = world.paperConfigPulseSpigot.loadChunks.tntEntities; // PaperSpigot
    }
    ...
    public EntityTNTPrimed(org.bukkit.Location loc, World world, double d0, double d1, double d2, EntityLiving entityliving) {
        ...
        if (world.paperConfigPulseSpigot.fixCannons) this.motX = this.motZ = 0.0F; // PaperSpigot - Fix cannons
    }
    ...
    public void t_() {
        if (world.spigotConfigPulseSpigot.currentPrimedTnt++ > world.spigotConfigPulseSpigot.currentPrimedTnt) { return; } // Spigot
        ...
        if (this.world.paperConfigPulseSpigot.tntEntityHeightNerf != 0 && this.locY > this.world.paperConfigPulseSpigot.tntEntityHeightNerf) {
            this.die();
        }
        // PaperSpigot end

        // PaperSpigot start - Remove entities in unloaded chunks
        if (this.inUnloadedChunk && world.paperConfigPulseSpigot.removeUnloaded.tntEntities) {
            this.die();
            this.fuseTicks = 2;
        }
        // PaperSpigot end
        ...
        if (this.fuseTicks-- <= 0) {
            ...
            this.W();
            if (!this.world.pulseConfig.optimizations.explosionParticles) return; // PulseSpigot
    ...
    private void explode() {
        ...
        if (world.paperConfigPulseSpigot.loadChunks.tntEntities) {
    ...
    public double f(double d0, double d1, double d2) {
        if (!world.paperConfigPulseSpigot.fixCannons) return super.f(d0, d1, d2);
    ...
    @Override
    public boolean aL() {
        return !world.paperConfigPulseSpigot.fixCannons && super.aL();
    }

    @Override
    public float getHeadHeight() {
        return world.paperConfigPulseSpigot.fixCannons ? this.length / 2 : 0.0F;
    }
    ...
    @Override
    public boolean W() {
        if (!world.paperConfigPulseSpigot.fixCannons) return super.W();
        ...
        if (this.inWater && this.world.pulseConfig.optimizations.syncTntInWater) { // PulseSpigot
