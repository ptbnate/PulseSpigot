...
public class BlockReed extends Block {
    ...
    public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
        if (world.getType(blockposition.down()).getBlock() == Blocks.REEDS || this.e(world, blockposition, iblockdata)) {
            if (world.isEmpty(blockposition.up())) {
                ...
                if (i < world.paperConfigPulseSpigot.maxGrowthHeight.reeds) { // PaperSpigot - Configurable max growth height for reed blocks) {
                    int j = ((Integer) iblockdata.get(BlockReed.AGE)).intValue();

                    if (j >= (byte) range(3, (world.growthOdds / world.spigotConfigPulseSpigot.growth.caneModifier * 15) + 0.5F, 15)) { // Spigot
                        // CraftBukkit start
    ...
    public boolean canPlace(World world, int x, int y, int z) { // Unicron
        ...
        if (block == this) {
            ...
            } while (world.getType(x, y - 1, z, enumdirection).getBlock().getMaterial() != Material.WATER); // PulseSpigot
