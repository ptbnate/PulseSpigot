...
public class EntitySlice<T> extends AbstractSet<T> {
    ...
    public <S> Iterable<S> c(final Class<S> oclass) {
        return new Iterable() {
            public Iterator<S> iterator() {
                ...
                if (list == null) {
                    return Collections.emptyIterator(); // PulseSpigot
                } else {
    ...
    public Iterator<T> iterator() {
        return this.e.isEmpty() ? Collections.emptyIterator() : Iterators.unmodifiableIterator(this.e.iterator()); // PulseSpigot
    }
