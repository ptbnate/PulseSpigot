...
public class EntityTrackerEntry {
    ...
    public java.util.Map<EntityPlayer, Boolean> trackedPlayerMap = new it.unimi.dsi.fastutil.objects.Object2BooleanOpenHashMap<>(); // PulseSpigot - Use fastutil map
    ...
    public void track(List<EntityHuman> list) {
        ...
        if (this.m % this.c == 0 || this.tracker.ai || this.tracker.getDataWatcher().a()) {
            ...
            if (this.tracker.vehicle == null) {
                ...
                if (this.m > 0 || this.tracker instanceof EntityArrow) { // PaperSpigot - Moved up
                    ...
                    if (j1 >= -128 && j1 < 128 && k1 >= -128 && k1 < 128 && l1 >= -128 && l1 < 128 && this.v <= 400 && !this.x) { // PulseSpigot - remove the useless onGround check
                        if ((!flag || !flag1) && !(this.tracker instanceof EntityArrow)) {
                            ...
                            } else if (flag1 && handleHeadRotation) { // PulseSpigot
                ...
                this.x = false;
            } else if (handleHeadRotation) { // PulseSpigot
            ...
            if (handleHeadRotation && Math.abs(i - this.i) >= 4) { // PulseSpigot
    ...
    private void b() {
        ...
        if (datawatcher.a()) {
            ...
            if (PulseConfig.get().settings.obfuscatePlayerHealth && this.tracker instanceof EntityHuman) {
                ...
                while (iterator.hasNext()) {
                    ...
                    // If the index is 6 (player health) we can replace it with an obfuscated value.
                    // We have to make sure the health is also over 0 otherwise death animations won't show
        ...
        if (entityplayer != this.tracker) {
            if (this.c(entityplayer)) {
                if (!this.trackedPlayers.contains(entityplayer) && (this.e(entityplayer) || this.tracker.attachedToPlayer)) {
                    ...
                    if (this.tracker instanceof EntityLiving && this.handleHeadRotation()) { // [SportPaper-0027] Fix head rotation packet spam // PulseSpigot
    ...
    // PulseSpigot start - Option to disable entity head rotation handling
    private boolean handleHeadRotation() {
        return this.tracker instanceof EntityHuman || this.tracker instanceof EntityVillager || this.tracker.world.pulseConfig.optimizations.handleHeadRotation;
    }
    // PulseSpigot end
}
