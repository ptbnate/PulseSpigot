...
public abstract class EntityProjectile extends Entity implements IProjectile {
    ...
    public EntityProjectile(World world) {
        ...
        float length = 0.25F;
        // PulseSpigot start
        if (this instanceof EntityEnderPearl && PulseSpigot.get().getMotionHandler().isSmallPearlHitbox()) {
            width = 0.2F;
            length = 0.2F;
        }
        // PulseSpigot start
    ...
    public void t_() {
        ...
        if (!this.world.isClientSide) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (entity1.ad() && (entity1 != entityliving || this.ar >= (this instanceof EntityPotion ? PulseSpigot.get().getMotionHandler().getPotionTime() : 5))) {
                    float f = 0.3F;
                    AxisAlignedBB axisalignedbb = entity1.getBoundingBox().grow((double) f, (double) f, (double) f);
                    MovingObjectPosition movingobjectposition1 = axisalignedbb.a(vec3d, vec3d1);

                    if (this instanceof EntityPotion && PulseSpigot.get().getMotionHandler().isSmoothPots() && movingobjectposition1 == null
        ...
        // PulseSpigot start
        if (PulseSpigot.get().getMotionHandler().isReducedSplash() && this instanceof EntityPotion) {
            Entity shooter = getShooter();
            if (shooter != null && shooter.isSprinting() && this.ar > 5) {
                double xDistance = Math.abs(shooter.locX - this.locX + this.motX);
                double yDistance = 2.0D - Math.abs(shooter.locY - this.locY + this.motY);
                double zDistance = Math.abs(shooter.locZ - this.locZ + this.motZ);

                double horizontalDistance = (xDistance + zDistance) / 2.0D;
                if (horizontalDistance < 0.75D && yDistance > -0.35D) {
                    die();
                    a(new MovingObjectPosition(shooter));
        ...
        if (movingobjectposition != null) {
            if (movingobjectposition.type == MovingObjectPosition.EnumMovingObjectType.BLOCK && this.world.getType(movingobjectposition.a()).getBlock() == Blocks.PORTAL) {
                ...
                // CraftBukkit start
                if (this.dead) {
                    org.bukkit.craftbukkit.event.CraftEventFactory.callProjectileHitEvent(this, movingobjectposition.entity, movingobjectposition.a(), movingobjectposition.direction); // PulseSpigot - Backport methods
                }
                // CraftBukkit end
            }
        }
