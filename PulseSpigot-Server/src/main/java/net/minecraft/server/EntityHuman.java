...
public abstract class EntityHuman extends EntityLiving {
    ...
    protected boolean d(DamageSource damagesource, float f) { // void -> boolean
        ...
        if (!this.isInvulnerable(damagesource)) {
            if (!damagesource.ignoresArmor() && this.isBlocking() && f > 0.0F) {
                f = (1.0F + f) * this.world.paperConfigPulseSpigot.playerBlockingDamageMultiplier; // PaperSpigot - Configurable damage multiplier for blocking;
            }
    ...
    public void attack(Entity entity) {
        ...
        if (entity.aD()) {
            if (!entity.l(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    boolean flag = !world.paperConfigPulseSpigot.gameMechanics.disablePlayerCrits && this.fallDistance > 0.0F && !this.onGround && !this.k_() && !this.V() && !this.hasEffect(MobEffectList.BLINDNESS) && this.vehicle == null && entity instanceof EntityLiving; // PaperSpigot
                    ...
                    if (flag2) {
                        if (entity instanceof EntityPlayer) {
                            if (((EntityPlayer) entity).knockbackProfile == null)
                                ((EntityPlayer) entity).knockbackProfile = PulseSpigot.get().getKnockbackHandler().getActiveProfile();;

                            KnockbackProfile knockbackProfile = ((EntityPlayer) entity).knockbackProfile;

                            knockbackProfile.handleEntityHuman(
                                    (EntityPlayer) this,
                                    entity,
                                    i,
                                    new Vector(victimMotX, victimMotY, victimMotZ)
                            );
                        }
                        ...
                        this.applyExhaustion(world.spigotConfigPulseSpigot.hunger.combatExhaustion); // Spigot - Change to use configurable value
                    } else if (flag1) {
                        entity.extinguish();
                    }
                }
    ...
    public void bF() {
        ...
        if (this.isSprinting()) {
            this.applyExhaustion(world.spigotConfigPulseSpigot.hunger.sprintExhaustion); // Spigot - Change to use configurable value
        } else {
            this.applyExhaustion(world.spigotConfigPulseSpigot.hunger.walkExhaustion); // Spigot - Change to use configurable value
        }
        ...
        if (this.vehicle == null) {
            ...
            if (this.a(Material.WATER)) {
                ...
                if (i > 0) {
                    ...
                    this.applyExhaustion(world.paperConfigPulseSpigot.playerExhaustion.swimming * (float) i * 0.01F); // PaperSpigot - Configurable swimming exhaustion
                }
            } else if (this.V()) {
                i = Math.round(MathHelper.sqrt(d0 * d0 + d2 * d2) * 100.0F);
                if (i > 0) {
                    this.a(StatisticList.l, i);
                    this.applyExhaustion(world.paperConfigPulseSpigot.playerExhaustion.swimming * (float) i * 0.01F); // PaperSpigot - Configurable swimming (diving) exhaustion
                }
            } else if (this.k_()) {
                if (d1 > 0.0D) {
    ...
    public void setEquipment(int i, ItemStack itemstack) {
        this.inventory.setEquipment(i, itemstack); // PulseSpigot
    }
