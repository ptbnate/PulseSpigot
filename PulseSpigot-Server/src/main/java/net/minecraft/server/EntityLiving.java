...
public abstract class EntityLiving extends Entity {
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        if (this.isInvulnerable(damagesource)) {
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if (flag && !(knockbackCancelled = world.paperConfigPulseSpigot.disableExplosionKnockback && damagesource.isExplosion() && this instanceof EntityHuman)) {
                // PaperSpigot end
    ...
    public void a(Entity entity, float f, double d0, double d1, DamageSource source) {
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            if (this instanceof EntityPlayer) {
                if (((EntityPlayer) this).knockbackProfile == null)
                    ((EntityPlayer) this).knockbackProfile = PulseSpigot.get().getKnockbackHandler().getActiveProfile();

                ((EntityPlayer) this).knockbackProfile.handleEntityLiving((EntityPlayer) this, entity, f, d0, d1);
                return;
            }
            ...
            if (source instanceof EntityDamageSourceIndirect) {
                if (((EntityDamageSourceIndirect) source).getProximateDamageSource() instanceof EntityFishingHook) {
                    horizontal = knockbackProfile.getRodHorizontal();
                    vertical = knockbackProfile.getRodVertical();
                } else if (((EntityDamageSourceIndirect) source).getProximateDamageSource() instanceof EntityArrow) {
                    horizontal = knockbackProfile.getArrowHorizontal();
                    vertical = knockbackProfile.getArrowVertical();
                } else if (((EntityDamageSourceIndirect) source).getProximateDamageSource() instanceof EntitySnowball) {
                    horizontal = knockbackProfile.getSnowballHorizontal();
                    vertical = knockbackProfile.getSnowballVertical();
                } else if (((EntityDamageSourceIndirect) source).getProximateDamageSource() instanceof EntityEgg) {
                    horizontal = knockbackProfile.getEggHorizontal();
                    vertical = knockbackProfile.getEggVertical();
                } else if (((EntityDamageSourceIndirect) source).getProximateDamageSource() instanceof EntityEnderPearl) {
                    horizontal = knockbackProfile.getPearlHorizontal();
                    vertical = knockbackProfile.getPearlVertical();
                }
            }

            this.ai = true;
            double f1 = MathHelper.sqrt(d0 * d0 + d1 * d1);
            double f2 = horizontal;
            this.motX /= 2.0D;
            this.motY /= 2.0D;
            this.motZ /= 2.0D;
    ...
    public void g(float f, float f1) {
        ...
        if (this.bM()) {
            ...
            if (this.V() && (!(this instanceof EntityHuman) || !((EntityHuman) this).abilities.isFlying)) {
                ...
                if (this.world.isClientSide && (!this.world.isLoaded((int) this.locX, (int) this.locZ) || !this.world.getChunkAtWorldCoords((int) this.locX, (int) this.locZ).o())) { // PulseSpigot - don't create new BlockPosition
                    if (this.locY > 0.0D) {
                        this.motY = -0.1D;
                    } else {
                        this.motY = 0.0D;
        ...
        if (!this.world.isClientSide) {
            ...
            for (int j = 0; j < 5; ++j) {
                ...
                if (!ItemStack.matches(itemstack1, itemstack)) {
                    // PulseSpigot start - Backport PlayerArmorChangeEvent
    ...
    protected void bL() {
        ...
        if (this.ad() && !list.isEmpty()) { // Spigot: Add this.ad() condition
            numCollisions -= world.spigotConfigPulseSpigot.maxEntityCollisions; // Spigot
            for (int i = 0; i < list.size(); ++i) {
                if (numCollisions > world.spigotConfigPulseSpigot.maxEntityCollisions) { break; } // Spigot
    ...
    public ScoreboardTeamBase getScoreboardTeam() {
        if (!this.world.tacoConfigPulseSpigot.allowNonPlayerEntitiesOnScoreboards && !(this instanceof EntityHuman)) return null; // TacoSpigot
