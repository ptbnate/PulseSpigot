...
public abstract class World implements IBlockAccess {
    ...
    public final List<Entity> entityList = new it.unimi.dsi.fastutil.objects.ObjectArrayList<Entity>() // PulseSpigot - use fastutil list
    {
        @Override
    ...
    public final List<EntityHuman> players = new it.unimi.dsi.fastutil.objects.ObjectArrayList<>(); // PulseSpigot - use fastutil list
    public final List<Entity> k = new it.unimi.dsi.fastutil.objects.ObjectArrayList<>(); // PulseSpigot - use fastutil list
    ...
    public IChunkProvider chunkProvider; // PulseSpigot - make public
    ...
    // PulseSpigot start
    public final PulseWorldConfig pulseConfig;
    public final WorldConfig spigotConfigPulseSpigot;
    public final PaperWorldConfig paperConfigPulseSpigot;
    public final TacoWorldConfig tacoConfigPulseSpigot;
    ...
    public Chunk getChunkIfLoaded(int x, int z) {
        return this.chunkProvider.getChunkIfLoaded(x, z); // PulseSpigot
    }
    ...
    protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider, MethodProfiler methodprofiler, boolean flag, ChunkGenerator gen, org.bukkit.World.Environment env) {
        ...
        // PulseSpigot start - modern config system
        this.worldData = worlddata;
        CraftServer server = this.getServer();
        this.pulseConfig = server.pulseConfig.getWorldConfig( worlddata.getName() );
        this.spigotConfigPulseSpigot = server.spigotConfig.getWorldConfig( worlddata.getName() );
        this.paperConfigPulseSpigot = server.paperConfig.getWorldConfig( worlddata.getName() );
        this.tacoConfigPulseSpigot = server.tacoConfig.getWorldConfig( worlddata.getName() );
        this.antiXray = this.spigotConfigPulseSpigot.getAntiXray();
        server.pulseSpigot.remapConfigs(this);
        // PulseSpigot end
        this.generator = gen;
        ...
        this.chunkTickList = new gnu.trove.map.hash.TLongShortHashMap( spigotConfigPulseSpigot.chunksPerTick* 5, 0.7f, Long.MIN_VALUE, Short.MIN_VALUE );
        ...
        //this.worldData = worlddata; // PulseSpigot - moved up
        ...
        // CraftBukkit end
        this.keepSpawnInMemory = paperConfigPulseSpigot.keepSpawnLoaded; // PaperSpigot
        timings = new co.aikar.timings.WorldTimingsHandler(this); // Spigot - code below can generate new world and access timings
        this.entityLimiter = new org.spigotmc.TickLimiter(spigotConfigPulseSpigot.maxTickTime.entity);
        this.tileLimiter = new org.spigotmc.TickLimiter(spigotConfigPulseSpigot.maxTickTime.tile);
    }
    ...
    // PulseSpigot end
        // PulseSpigot start - don't look chunk twice
        Chunk chunk = this.getChunkIfLoaded(x >> 4, z >> 4);
        if (chunk != null) {
        // PulseSpigot end
            try {
                return chunk.getBiome(x, z, this.worldProvider.m()); // PulseSpigot
                ...
                crashreportsystemdetails.a("Location", new Callable() {
                    public String a() throws Exception {
                        return CrashReportSystemDetails.a(x, z); // PulseSpigot
                    }
            ...
            return this.worldProvider.m().getBiome(x, z, BiomeBase.PLAINS); // PulseSpigot
        }
    }
    ...
    public void d(BlockPosition blockposition, final Block block) {
        if (!this.isClientSide) {
            ...
            try {
                ...
                if (world != null && ((WorldServer) this).hasPhysicsEvent && (this.tacoConfigPulseSpigot.redstoneFireBlockPhysicsEvent || !(block instanceof BlockRedstoneWire || block instanceof BlockRedstoneTorch || block instanceof BlockRepeater))) { // Paper // TacoSpigot - add config to disable redstone physics event
    ...
    public final int c(int x, int y, int z, boolean flag) {
        if (isInWorldBounds(x, z)) { // PulseSpigot
    ...
    public int b(int i, int j) {
        if (isInWorldBounds(i, j)) { // PulseSpigot
            // PulseSpigot start - don't look chunk twice
    ...
    // PulseSpigot end
            ...
            // PulseSpigot end
                ...
                chunk.a(enumskyblock, x, y, z, i); // PulseSpigot
                //this.n(blockposition); // PulseSpigot - unused
            }
        }
    }

    public void n(BlockPosition blockposition) {
        // PulseSpigot start - unused
        /*for (int i = 0; i < this.u.size(); ++i) {
    ...
    // Spigot end
        ...
        // CraftBukkit end
        if (!this.isValidLocation(x, y, z)) { // PulseSpigot
            return Blocks.AIR.getBlockData();
        } else {
            Chunk chunk = this.getChunkAtWorldCoords(x, z); // PulseSpigot
    ...
    public MovingObjectPosition rayTrace(Vec3D vec3d, Vec3D vec3d1, boolean flag, boolean flag1, boolean flag2) {
        if (!Double.isNaN(vec3d.a) && !Double.isNaN(vec3d.b) && !Double.isNaN(vec3d.c)) {
            if (!Double.isNaN(vec3d1.a) && !Double.isNaN(vec3d1.b) && !Double.isNaN(vec3d1.c)) {
                ...
                IBlockData iblockdata = this.getTypeIfLoaded(l, i1, j1); // Paper // PulseSpigot - don't use BlockPositon
                if (iblockdata == null) return null; // Paper
                Block block = iblockdata.getBlock();
                BlockPosition blockposition = new BlockPosition(l, i1, j1); // PulseSpigot
    ...
    public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
        ...
        // Spigot start
            ...
            double radius = spigotConfigPulseSpigot.mergeRadius.exp;
            if (radius > 0) {
    ...
    public List<AxisAlignedBB> getCubes(Entity entity, AxisAlignedBB axisalignedbb) {
        ...
        if (!this.pulseConfig.optimizations.entitiesCollisions && (entity instanceof EntityHuman || entity instanceof EntityInsentient || entity instanceof EntityArmorStand)) return arraylist; // PulseSpigot
        if (this.pulseConfig.optimizations.optimizeArmorStands && entity instanceof EntityArmorStand) return arraylist; // PulseSpigot
            ...
            for (int l1 = i1; l1 < j1; ++l1) {
                if (this.isLoaded(blockposition_mutableblockposition.c(k1, 64, l1))) {
                    for (int i2 = k - 1; i2 < l; ++i2) {
                        ...
                        if (isInWorldBounds(k1, l1)) { // PulseSpigot
        ...
        for (tileTickPosition = 0; tileTickPosition < tileEntityList.size(); tileTickPosition++) { // PaperSpigot - Disable tick limiters
            ...
            // PulseSpigot end
                    try {
                        ...
                        // PaperSpigot start - Prevent tile entity and entity crashes
                        String msg = "TileEntity threw exception at " + tileentity.world.getWorld().getName() + ":" + x + "," + y + "," + z; // PulseSpigot
                        System.err.println(msg);
            ...
            if (tileentity.x()) {
                ...
                // PulseSpigot start - don't look chunk twice
                Chunk chunk = this.getChunkIfLoaded(x >> 4, z >> 4); // PulseSpigot
                if (chunk != null) {
    ...
    public void entityJoinedWorld(Entity entity, boolean flag) {
        ...
        if (!org.spigotmc.ActivationRange.checkIfActive(entity)) {
            ...
            if (!this.isChunkLoaded(i, j, true) && ((entity instanceof EntityEnderPearl && this.paperConfigPulseSpigot.removeUnloaded.enderpearls) ||
                    (entity instanceof EntityFallingBlock && this.paperConfigPulseSpigot.removeUnloaded.fallingBlocks) ||
                    (entity instanceof EntityTNTPrimed && this.paperConfigPulseSpigot.removeUnloaded.tntEntities))) {
    ...
    public boolean a(AxisAlignedBB axisalignedbb, Material material, Entity entity) {
        ...
        if (!this.isAreaLoaded(i, k, i1, j, l, j1, true)) {
            ...
            for (int k1 = i; k1 < j; ++k1) {
                for (int l1 = k; l1 < l; ++l1) {
                    for (int i2 = i1; i2 < j1; ++i2) {
                        //blockposition_mutableblockposition.c(k1, l1, i2); // PulseSpigot - moved down
                        IBlockData iblockdata = this.getTypeIfLoaded(k1, l1, i2); // PulseSpigot
                        if (iblockdata == null) return false; // PulseSpigot
                        Block block = iblockdata.getBlock();

                        if (block.getMaterial() == material) {
                            double d0 = ((float) (l1 + 1) - BlockFluids.b(iblockdata.get(BlockFluids.LEVEL))); // PulseSpigot

                            if ((double) l >= d0) {
                                flag = true;
                                blockposition_mutableblockposition.c(k1, l1, i2); // PulseSpigot
    ...
    public boolean capturedTileEntitiesEmpty = false; // PulseSpigot
    public Map<BlockPosition, TileEntity> capturedTileEntities = Maps.newHashMap();

    public TileEntity getTileEntity(int x, int y, int z) { // PulseSpigot
        if (!this.isValidLocation(x, y, z)) { // PulseSpigot
            return null;
        } else {
            // CraftBukkit start
            BlockPosition blockposition = new BlockPosition(x, y, z); // PulseSpigot
            // Paper start - Optimize Captured TileEntity Lookup
            TileEntity tileentity = null;
            if (!this.capturedTileEntitiesEmpty && !capturedTileEntities.isEmpty() && (tileentity = capturedTileEntities.get(blockposition)) != null) { // PulseSpigot
                return tileentity;
            // Paper end
            }
            // CraftBukkit end
    ...
    public void setTileEntity(BlockPosition blockposition, TileEntity tileentity) {
        if (tileentity != null && !tileentity.x()) {
            ...
            if (captureBlockStates) {
                ...
                this.capturedTileEntitiesEmpty = false; // PulseSpigot
                return;
            }
            // CraftBukkit end
            if (this.M) {
    ...
    protected void D() {
        ...
        // Spigot start
        int optimalChunks = spigotConfigPulseSpigot.chunksPerTick;
        ...
        if (spigotConfigPulseSpigot.randomLightUpdates && !this.players.isEmpty()) { // Spigot
    ...
    protected void a(int i, int j, Chunk chunk) {
        if (!this.paperConfigPulseSpigot.disableMoodSounds && this.L == 0 && !this.isClientSide) { // PaperSpigot - Disable mood sounds
            ...
            l += i;
            i1 += j;
            if (block.getMaterial() == Material.AIR && this.k(x, y, z) <= this.random.nextInt(8) && this.b(EnumSkyBlock.SKY, x, y, z) <= 0) { // PulseSpigot
    ...
    public boolean c(EnumSkyBlock enumskyblock, BlockPosition blockposition, Chunk chunk, List<Chunk> neighbors) { // PaperSpigot
        ...
        if (chunk == null /*|| !chunk.areNeighborsLoaded(1)*/ /*!this.areChunksLoaded(blockposition, 17, false)*/) {
            ...
            if (chunk.world.paperConfigPulseSpigot.useAsyncLightning) {
    ...
    public Entity getEntity(int i) { return  this.a(i); } // PulseSpigot- OBFHELPER
    ...
    public void updateAdjacentComparators(BlockPosition blockposition, Block block) {
        ...
        while (iterator.hasNext()) {
            ...
            // PulseSpigot start - dont look chunk twice
