...
public class EntityFallingBlock extends Entity {
    ...
    public EntityFallingBlock(org.bukkit.Location loc, World world) {
        super(world);
        sourceLoc = loc;
        this.loadChunks = world.paperConfigPulseSpigot.loadChunks.fallingBlocks; // PaperSpigot
    }
    ...
    public void t_() {
        ...
        if (block.getMaterial() == Material.AIR) {
            ...
            if (this.inUnloadedChunk && world.paperConfigPulseSpigot.removeUnloaded.fallingBlocks) {
                this.die();
            }
            // PaperSpigot end

            // PaperSpigot start - Drop falling blocks above the specified height
            if (this.world.paperConfigPulseSpigot.fallingBlockHeightNerf != 0 && this.locY > this.world.paperConfigPulseSpigot.fallingBlockHeightNerf) {
            ...
            if (!this.world.isClientSide) {
                ...
                if (this.onGround) {
                    ...
                    if (this.world.getType(blockposition).getBlock() != Blocks.PISTON_EXTENSION) {
                        this.die();
                        if (!this.e) {
                            if (this.world.a(block, blockposition, true, EnumDirection.UP, (Entity) null, (ItemStack) null) && !BlockFalling.canFall(this.world, blockposition.down()) /* mimic the false conditions of setTypeIdAndData */ && World.isInWorldBounds(blockposition) && blockposition.getY() >= 0 && blockposition.getY() < (this.world.tacoConfigPulseSpigot.disableFallingBlockStackingAt256 ? 255 : 256) && this.world.getType(blockposition) != this.block) { // TacoSpigot
    ...
    public float getHeadHeight() {
        return world.paperConfigPulseSpigot.fixCannons ? this.length / 2 : super.getHeadHeight();
    }
    // PaperSpigot end
}
