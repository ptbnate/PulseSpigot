...
public class TileEntityHopper extends TileEntityContainer implements IHopper, IUpdatePlayerListBox {
    ...
    public boolean m() {
        if (this.world != null && !this.world.isClientSide) {
            if (!this.n() && BlockHopper.f(this.u())) {
                ...
                if (flag) {
                    this.d(world.spigotConfigPulseSpigot.ticksPer.hopperTransfer); // Spigot
                    this.update();
                    return true;
                }
            }
            // PaperSpigot start
            if (world.paperConfigPulseSpigot.useHopperCheck && !this.world.tacoConfigPulseSpigot.hopper.pushBased && !this.n()) { // TacoSpigot - dont use hopper check in push mode
                this.d(world.spigotConfigPulseSpigot.ticksPer.hopperCheck);
            }
            // PaperSpigot end
            return false;
        } else {
            ...
            if (this.a(iinventory, enumdirection)) {
                ...
                for (int i = 0; i < this.getSize(); ++i) {
                    if (this.getItem(i) != null) {
                        ...
                        CraftItemStack oitemstack = CraftItemStack.asCraftMirror(this.splitStack(i, world.spigotConfigPulseSpigot.hopperAmount)); // Spigot
    ...
    public static boolean a(IHopper ihopper) {
        ...
        if (ihopper.getWorld().tacoConfigPulseSpigot.hopper.pushBased&& ihopper instanceof TileEntityHopper) {
    ...
    public static boolean acceptItem(IHopper ihopper, IInventory iinventory) {
        ...
        } else if (!ihopper.getWorld().tacoConfigPulseSpigot.hopper.pushBased || !(ihopper instanceof TileEntityHopper)) { // TacoSpigot - only search for entities in 'pull mode'
    ...
    private static boolean a(IHopper ihopper, IInventory iinventory, int i, EnumDirection enumdirection) {
        ...
        if (itemstack != null && b(iinventory, itemstack, i, enumdirection)) {
            ...
            CraftItemStack oitemstack = CraftItemStack.asCraftMirror(iinventory.splitStack(i, ihopper.getWorld().spigotConfigPulseSpigot.hopperAmount)); // Spigot
            // TacoSpigot start - add an option to turn of InventoryMoveItemEvent
            final org.bukkit.inventory.ItemStack stack;
            ...
            if (event.isCancelled()) {
                ...
                if (ihopper instanceof TileEntityHopper) {
                    ((TileEntityHopper) ihopper).d(ihopper.getWorld().spigotConfigPulseSpigot.ticksPer.hopperTransfer); // Spigot
                } else if (ihopper instanceof EntityMinecartHopper) {
                    ((EntityMinecartHopper) ihopper).m(ihopper.getWorld().spigotConfigPulseSpigot.ticksPer.hopperTransfer / 2); // Spigot
                }
                return false;
            }
            // TacoSpigot start
            ...
            if (flag) {
                if (iinventory instanceof TileEntityHopper) {
                    ...
                    if (tileentityhopper.o()) {
                        tileentityhopper.d(tileentityhopper.world.spigotConfigPulseSpigot.ticksPer.hopperTransfer); // Spigot
                    }
    ...
    public static IInventory b(World world, double d0, double d1, double d2) {
        ...
        if (object == null && world.getChunkIfLoaded(i >> 4, k >> 4).getItemCount(blockposition) > 0 && !org.bukkit.craftbukkit.util.CraftMagicNumbers.getMaterial(block).isOccluding()) { // PulseSpigot
