...
public abstract class EntityInsentient extends EntityLiving {
    ...
    protected void a(EntityItem entityitem) {
        ...
        if (i > -1) {
            ...
            // PulseSpigot start - Backport EntityPickupItemEvent
    ...
    protected void D() {
        if (this.persistent) {
            ...
            if (entityhuman != null) {
                ...
                if (d3 > this.world.paperConfigPulseSpigot.despawnRanges.hard) { // CraftBukkit - remove isTypeNotPersistent() check // PaperSpigot - custom despawn distances
                    this.die();
                }

                if (this.ticksFarFromPlayer > 600 && this.random.nextInt(800) == 0 && d3 > this.world.paperConfigPulseSpigot.despawnRanges.soft) { // CraftBukkit - remove isTypeNotPersistent() check // PaperSpigot - custom despawn distance
                    this.die();
                } else if (d3 < this.world.paperConfigPulseSpigot.despawnRanges.soft) { // PaperSpigot - custom despawn distances
                    this.ticksFarFromPlayer = 0;
                }
            }
    ...
    @Override
    protected void bL() {
        if (this.world.pulseConfig.optimizations.entitiesCollisions)
        super.bL();
    }
    // PulseSpigot end
}
