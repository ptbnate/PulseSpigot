...
public class CraftBlock implements Block {
    ...
    public boolean setTypeIdAndData(final int type, final byte data, final boolean applyPhysics) {
        ...
        BlockPosition position = new BlockPosition(x, y, z);
        net.minecraft.server.World nmsWorld = chunk.getHandle().getWorld(); // PulseSpigot
        if (applyPhysics) {
            return nmsWorld.setTypeAndData(position, blockData, 3); // PulseSpigot
        } else {
            boolean success = nmsWorld.setTypeAndData(position, blockData, 2); // PulseSpigot
            if (success) {
                nmsWorld.notify(x, y, z); // PulseSpigot
            }
            return success;
        }
    }
    ...
    public int getTypeId() {
        return CraftMagicNumbers.getId(chunk.getHandle().getBlockData(this.x, this.y, this.z).getBlock()); // PulseSpigot
    }

    public byte getLightLevel() {
        return (byte) chunk.getHandle().getWorld().getLightLevel(this.x, this.y, this.z); // PulseSpigot
    }

    public byte getLightFromSky() {
        return (byte) chunk.getHandle().getBrightness(EnumSkyBlock.SKY, this.x, this.y, this.z); // PulseSpigot
    }

    public byte getLightFromBlocks() {
        return (byte) chunk.getHandle().getBrightness(EnumSkyBlock.BLOCK, this.x, this.y, this.z); // PulseSpigot
    }
